	...
	/*
		...
		3. 클래스 구조 : 헤더, 멤버필드, 멤버메서드, 생성자
		   1) 헤더 : [접근제한자] [클래스종류] class 클래스이름{}
		      가) 접근제한자 : 클래스, 멤버필드, 멤버메서드, 생성자 모두 맨 앞에 사용가능 (생략가능)
		         ...
		         - protected : 같은 패키지에서 접근가능, 다른 패키지일 때는 상속관계에서만 가능
		      나) 클래스종류 : 일반적인 클래스는 생략한다.
		                   특정 클래스는 해당하는 클래스에게만 예약어를 사용(final, abstract, ...)
		      다) class : 클래스임을 나타내는 예약어
		      라) 클래스이름 : 클래스 = 파일이름 = 저장이름 = 생성자이름
		...
		5. 멤버메서드 : 동작, 기능
		   ...
		   1) 메서드 구성
		      ...
		      // 메인 메서드 : 해당 메서드를 실핸한 후에 호출한 JVM 에게는 되돌아 가는데 아무것도 가져가지 않는다.
		      public static void main(String[] args ){
		      라) 메서드이름 : 첫글자 소문자, 두 단어이상 일때는 단어 첫 글자들은 대문자
		      			   ([인자=매개변수]}앞에 이름 붙는다.
		      			   
		      마) 인자 = 매개변수 : 메서드가 실핼할 때 필요한 정보를 외부에서 받기 위한 통로
		         ** 메서드 이름이 같아도 인자 수나 인자 자료형이 다른 다른 메서드 취급한다.
		         ** 오버로딩(중복정의) : 클래스안에 같은 이름을 가진 메서드가 여러개 정의되어 있는 것
		 ...
		 6. 생성자 : 클래스가 객체로 만들어질 때 한번 호출된다.
			...
			new 예약어는 사용하면 무조건 heap 메모리에 사용영역을 만든다.
			즉, 인스턴스, 객체 생성을 의미한다.
	    ...
	    8. static 과 instance : 멤버필드, 멤버메서드에 공통으로 해당 되는 사항
	       ...
	       2) static
	          - 객체 생성과 상관없이 미리 만들어진 멤버필드와 멤버메서드이다.
